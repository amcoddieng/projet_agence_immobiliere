...
<div class="container mx-auto bg-white pt-6 pb-6 px-4 mb-3" data-aos="fade-up" data-aos-duration="500">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <div class="bg-white border overflow-hidden shadow-md hover:shadow-lg hover:scale-105 transition-transform" data-aos="fade-up" data-aos-duration="500">
            ...
            <a href="{{route('admin.bien.show',['id' => $villa->id])}}">
            @if(!empty($villa->images))
                    ...
                    @if (is_array($images) && count($images) > 0)
                        <img src="{{ asset('storage/'.$images[0]) }}" alt="Image de {{ $villa->titre }}" class="object-cover w-full h-40">
                    @else
             ...
             <div class="p-4">
                <h5 class="text-lg font-semibold mb-2 text-gray-800">{{ $villa->titre }} a : {{ $villa->adresse}} </h5>
                <p class="text-sm text-gray-600">Prix {{ $villa->cycle }} : {{ $villa->prix }}</p>
            </div>
            </a>
        </div>
        @endforeach
